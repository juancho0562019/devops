...
public class ConfiguracionDocumentoSede : IEntityTypeConfiguration<DocumentoSede>
{
    public void Configure(EntityTypeBuilder<DocumentoSede> builder)
    {
        ...
        builder.Property(x => x.SedeId).ValueGeneratedNever();
      
        builder.HasKey(@"Id");
        builder.HasOne(x => x.Sede).WithMany(op => op.Documentos).HasForeignKey(v => v.SedeId).IsRequired(true);
